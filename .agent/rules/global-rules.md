...
- **Web (App)**: Next.js (App Router) powered by **OpenNext** on **Cloudflare Workers** (Path: `apps/app/`)
...
## UI/UX Design Principles (引き算の美学と一貫性)
...
- **UI Language (完全英語化の原則)**:
  - ユーザー向けに表示されるWebダッシュボードや拡張機能のUIテキスト（ボタン、プレースホルダー、案内文、エラーメッセージなど）は、**すべて自然な英語で実装**してください。日本語での仮置きや混在は禁止です。
- **Product Naming (ブランド名)**:
  - プロダクト名は常に全小文字の **`sitecue`** で統一してください（`SiteCue` などの表記揺れは禁止）。
- **Iconography (アイコンの統一)**:
  - UIの視覚的な装飾には、仮置きであっても絵文字（🐦、📝等）を使用しないでください。必ず `lucide-react` のアイコンを使用し、スタイリッシュで統一感のあるデザインを保ってください。
...
- **型チェックとパス解決の徹底 (Type Checking)**:
  ...
  - コードの修正後は、Biomeによるチェックに加えて、必ず各ワークスペース（例: `apps/extension/`）で `bun run typecheck` (`tsc --noEmit`) を実行し、モジュール解決エラーや型の不整合が発生していないか確認してください。
