...
- **PCサイドバーのカプセル型外殻と真円島の階層設計**:
  PC版サイドバーの外殻には `bg-action rounded-full` を適用して滑らかなカプセル型（スタジアム型）を構築し、内部には適切なパディングを介して3つの独立した「白い真円の島コンテナ（`bg-base-surface rounded-full p-2`）」を垂直配置する。中のアイコンやボタン自体も真円（`rounded-full`）を徹底し、将来的なアイテム増設に対応する拡張性と美しい立体感を厳格に保持する。

- **モバイルボトムナビのFlexシンメトリーとスペース最大化**:
  画面幅の狭いモバイル環境において中央の白い島を息苦しくなく最大化するため、等幅Grid（`grid-cols-3`）の利用は禁止とする。左右の端を等幅の固定ブロック（`w-12 flex justify-start/end`）で縛るFlexシンメトリー構造を採用し、中央の島が自由に拡張できるスペースを最大化（`max-w-[220px]`）する。
  中央の3つの主要ボタンは押しやすさを担保するため、確実に `w-12 h-12` (48px) の真円ヒットエリアを死守し、左端のホームボタンは背景なしの白透過（`text-white/60`）の `w-9 h-9` にスケールダウンして、圧倒的な機能プライオリティ（質量差）を表現すること。
