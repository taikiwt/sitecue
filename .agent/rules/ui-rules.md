...
アプリケーション全体における各エンティティの遷移先URLは以下を唯一の正（SSOT）として厳格に組み立てる。AIはこれらを推測でねつ造したり、古いパラメータ構造に先祖返りさせてはならない。
...
アプリケーション全体（Launchpad, タイムライン等）において、ドメインや外部URLのファビコンを描画・表示する際は、生の `<img>` タグを個別に配置したり、推測で外部APIを乱打することを厳禁とする。
