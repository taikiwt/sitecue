...
- **Extension**: React + Vite + Tailwind CSS (Chrome Extension Manifest V3)
  ...
  - 🛡️ **Permissions Strategy (権限戦略 - 引き算の美学)**:
    ...
    - **Dynamic Permissions (Safety First)**: `<all_urls>` を `manifest.json` の `host_permissions` に直接記述することは審査リスクの増大とユーザーの警戒感を招くため**絶対禁止**とする。対象ページへのアクセスが必要な場合は、必ず `optional_host_permissions` に定義し、処理実行の直前に `chrome.permissions.request` を用いて、ユーザーのアクションに応じた動的な権限要求を行うこと。
