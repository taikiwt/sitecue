...
- **NoteInput カプセル型インライン構造:** NoteとDiaryの各入力ブロックは、それ自体が [左アイコン] [中央コンテンツ] [右送信ボタン] を内包する `grid grid-cols-[auto_1fr_auto] items-center gap-3` のカプセル型の美しい浮き島にすること。全体は薄グレー背景（`bg-base-surface`）の上に上下に並列配置する。
