...
- **Extension**: React + WXT + Tailwind CSS (Chrome Extension Manifest V3)
  ...
  - **Communication Pattern**: Supabase JS Client をラップした 専用DAL (utils/dal.ts) を経由して DBと直接通信する。
  ...
  - ⛔ **Prohibited**: apps/extension/ から自社DBのCRUD操作のために apps/api/ (Hono) を経由することは禁止（直接Supabaseクライアントを叩くのではなく、必ずExtension用DALを経由すること）。
