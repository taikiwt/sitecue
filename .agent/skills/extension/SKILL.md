...
- **無変更保存ガード（防壁）:** 日記の本文テキストとトピック配列の双方が、DBから取得した初期値と完全に一致している（変更がない）場合は、自動保存のミューテーションをバイパスし、無駄なネットワークミューテーションを物理的に遮断すること。
- **サイレント自動保存＆オートフォーカス:** 手動の保存/編集ボタンは完全に排除し、通常時のプレビュー部分のクリック判定（インタラクティブ要素除外）による自動フォーカス移行、および onBlur / 3秒デバウンスによるサイレントクラウド保存を徹底すること。クラウド同期前にSidePanelが終了した場合の `chrome.storage.local` を介した自動復元マージライフサイクルを絶対維持すること。
- **プレビュー画面でのテキスト選択（ドラッグ）の保護:** プレビュー画面でのテキスト選択（ドラッグ）を保護するため、MouseDownとMouseUpの座標差分が5px未満のときのみシングルクリックとして編集を起動し、5px以上の場合はドラッグとみなして起動をガードすること。また、プレビュー時のカード全体のカーソルはデフォルト（cursor-default）を維持し、余計なホバーエフェクトを加えず通常の閲覧モードとして佇ませること。
...
- **閲覧モードと編集モードのイベント境界と余白のクローズ利用:** 閲覧モード時の編集移行イベントは、パディング余白（p-6）を避け、インナーのコンテンツスタックにのみバインドすること。編集モード時は、スクロールコンテナ自身に MouseUp による保存・プレビュー復帰イベントを付与し、入力フォームやエディタ要素内部は stopPropagation によりイベントを隔離し、パディング余白やヘッダーのクリックによってスムーズに閲覧モードに戻れること。
...
## Fractional Indexing 移植 ＆ ゲストクライアント安全防壁規約
- **Fractional Indexing による順序計算:** D&D並び替え完了時の順序（`sort_order`）計算は、App Basecamp側で実証済みの「初期インデックス vs 最終インデックス」の差分比較による Fractional Indexing を標準とする。隣接要素と同値衝突時は、ドラッグ元と先の高低差から移動方向を特定し、極小オフセット（`OFFSET = 0.0001`）を加減算して安全にスナップバックを防止する。
- **ゲストモード時のDAL安全ガード:** ゲストモード（`authStatus.mode === 'guest'`）時は、共通DAL（`createNoteEntity`等）の内部で実行される複雑なSupabaseメソッドチェーンがローカルクライアント（`localClient`）上で例外クラッシュを引き起こすのを防ぐため、DAL呼び出しをバイパスする。`localClient` に対する直接操作（`from('sitecue_notes')` 等）を行い、作成・更新・削除などのクエリ実行を透過的にエミュレートする。

## 📐 拡張機能ヘッダー等高クロスフェード規約 (CLS & Flicker Protection)
- **境界線の一元化:** 閲覧モードと編集モードを切り替えるコンポーネントのヘッダーは、個別の内包要素に `border-b` を持たせてはならない。必ず最外殻の共通Stickyコンテナ自体の底辺に `border-b border-base-border/20` を一元設定すること。
- **等高設計によるCLS防止:** モード移行時の物理的な高さブレや線のダブりによるモヤモヤ（フリッカー）を完全に排除するため、通常時の1段ヘッダーと、編集時の1段目（決定行）の最小高さを完全に一致（`min-h-[40px]` 等）させ、シームレスなクロスフェードを実現すること。

## 🎯 アコーディオン縮小時における視界復帰の掟 (Context Preservation)
- **迷子防止スクロール:** 長文コンテンツを `[ Show less ]`（折りたたみ）ボタンで縮小した際、カード要素が画面上部外に見切れてユーザーが現在地を見失うのを防ぐため、縮小契機（`true` -> `false` への反転時）で必ず対象要素に対し `scrollIntoView({ behavior: "smooth", block: "nearest" })` を発火させること。
- **テスト環境用ガード（防壁）:** `jsdom` や `happy-dom` などのブラウザAPIが完全再現されていないテスト環境下での例外クラッシュを物理的に防ぐため、実行直前には必ず `typeof element.scrollIntoView === "function"` の存在判定（防御的コーディング）を絶対に挟むこと。
