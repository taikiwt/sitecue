...
- **Diary文字数オーバー時の即時ロールバック:** 日記の保存時（`handleSaveDiaryEdit`）に `SHARED_LIMITS.DIARY_LENGTH` を超える入力があった場合、および書き込み失敗時は、編集用 State (`editDiaryContent`) とローカルバッファを即座に DB 由来の最新正常データ (`diaryData.content`) へ巻き戻し（ロールバック）、壊れた長文テキストがメモリ上に残存してエラーが再発するのを物理遮断すること。
- **Diary一時キャッシュ（`diary-temp-*`）の完全破棄と復元時バリデーション:** 日記編集時に文字数上限（`SHARED_LIMITS.DIARY_LENGTH`）を超えた場合や保存エラー発生時は、デバウンスタイマーを即座に破棄（`clearTimeout`）し、`chrome.storage.local.remove(`diary-temp-${selectedDiaryDate}`)` でストレージ領域から汚染データを物理除去すること。また、キャッシュからの復元時にも文字数上限チェックを行い、超過テキストは復元を拒否して即座にストレージから削除し、DB由来の正常データを優先採用すること。
- **Diary編集保存の楽観的UI（Optimistic UI）原則**: 日記の編集保存（`handleSaveDiaryEdit`）時は、DB通信のレスポンスを待たずに 0ms で `setIsEditingDiary(false)` を実行して即座に閲覧モードへ切り替えること。同時に `queryClient.setQueryData` でキャッシュを先行更新し、通信失敗時のみ直前データへのロールバックとトースト通知を行うこと。
- **Diary完全サイレント保存 ＆ フリッカー防止スケルトン原則**: スケルトン表示は初回データ未保持のローディング時（Cold State）のみ発動し、フリッカー防止のため最低200ms (`SKELETON_HOLD_DURATION = 200`) 間保持すること。2回目以降の日付切り替えや編集復帰時（Warm State）はスケルトンを一切出さず、`useDeferredValue` で Markdown 描画を非同期化して 0ms で即時切り替えること。また保存時はヘッダー等の「Saving...」表示を一切出さずバックグラウンドで完全サイレント保存を行うこと。
- **Diaryデータ全体のアトミック遅延同期原則**: 閲覧モード復帰や日付切り替え時の非同期描画において、`useDeferredValue` は `content` 単体ではなく `diaryData` オブジェクト全体（`deferredDiaryData = useDeferredValue(diaryData)`）に適用すること。これによりトピックと本文の表示タイミングをアトミック（不可分）に同期させ、トピックだけが先に消える表示不整合を100%防止すること。
- **文字数差分しきい値（500文字）に基づく動的スケルトン原則**: `diaryData`（新データ）と `deferredDiaryData`（旧データ）の本文文字数差分（`Math.abs(rawContent.length - deferredContent.length)`）が **500文字以上** あり、かつ同一日付内での編集保存時（`isDateChanged` が false）のみ、旧テキスト露出を防ぐためスケルトン表示（最低200ms維持）を発動させること。日付切り替え時は文字数差分に関わらずスケルトンを完全バイパスし 0ms 即時切り替えを行うこと。500文字未満の微修正時もスケルトンを出さず 0ms で即座に閲覧モードへ復帰すること。
...
- **Quick Note 送信ボタンの上限連動ルール**: Quick Note（スクラッチパッド）入力欄自体の入力・保持には文字数制限を設けず自由にスクラッチ可能とする。ただし「Note」送信ボタンは Note プラン上限（Free: 1万字 / Pro: 3万字）超えで `disabled` 化し、「Diary」送信ボタンは Diary プラン上限（Free: 5万字 / Pro: 10万字）超えで `disabled` 化して、不整合な送信をスマートに遮断すること。
...
## 拡張機能アセット（フォント等）のローカル同梱ルール
- Webフォントを導入する際は、必ずサブセット化されたローカルフォントファイルをアセットとして同梱（例: `apps/extension/src/assets/fonts/`）し、`index.css` から相対パスで参照すること。
- CLS（レイアウトシフト）を引き起こさないよう、等高・固定高設計およびローカルでの一瞬のフォント読み込みを保証すること。

## 極上エディタ入力補助規約
- インタラクション（Tabインデント、改行自動リストバレット、ペア閉じ）は開発者の執筆リズムの保護を最優先（先読み貫通、自動離脱、連動削除）すること。
- リスト行（`- `, `* `, `1. `等）でTabを押した際は、カーソル位置に関わらず常に行頭を操作して「リスト記号ごとインデント（入れ子化）」を保証すること。
- 空のリストマーカーでEnterを押して離脱する際は、単に文字を削除するのではなくクリーンな改行（`\n`）へ置換することで、カーソルを安全に次行の先頭へスライド着地させること。
- バッククォート（` ` `）の自動閉じは、コードブロック（` ` ` ` `）の執筆の邪魔をしないようアシスト対象から完全に除外（引き算）すること。
