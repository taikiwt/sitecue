...
- **物理的制約と強力な折り返し (Robust Word Wrapping)**: 長大なURLやスペースのない連続文字列（Raw Data）によって、Flex/Gridコンテナが意図せず膨張し、レイアウトを破壊（横スクロールの発生）することを防ぐため、コンシューマー側のコンテナには必ず `min-w-0`, `w-full`, `overflow-hidden` などの物理的制約を課すこと。また、MarkdownRenderer などの子要素に対しては `[overflow-wrap:anywhere]` や `break-all` を注入し、あらゆる箇所での強制的な折り返しを貫通させること。
