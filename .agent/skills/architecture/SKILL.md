...
1. **DBのCRUD操作:** Extension・App Basecampともに、自社のDB（sitecue_notes 等）の読み書きは Hono API を経由させず、必ず各環境の専用DAL（lib/dal/ または utils/dal.ts）を経由して Supabase と通信すること（RLSによる保護を活用するため）。
