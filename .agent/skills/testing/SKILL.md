...
- **モック定義の型厳格化**: テストのモック定義（`vi.mock` や `vi.mocked` 等）において `as any` は絶対に使用しないこと。必ず元のフックやストアの型（`ReturnType<typeof useXXX>` や `Partial<T>` などのユーティリティ型）、または実際のステート型（`UserState` 等）を厳密に適用し、TypeScriptのエラーが出ないよう安全にモックを構築すること。
