...
## 🎯 スマート・テスト・セレクション (Smart Test Selection)

モノレポ環境でのテスト実行時間を短縮し、開発のテンポを維持するため、以下の基準で実行対象を選択すること。

- **UIのみの微修正**: レイアウトの微調整やテキストの変更のみで、ロジックに変更がない場合は、**テスト実行 0件** を許容する。
- **ロジック/状態の変更**: Hooksの修正や、新しいURLパラメータの導入など、振る舞いに変更がある場合は、影響を受けるテストを**最大3つ**に絞って実行する。
- **リファクタリング**: 外部仕様を変えずに内部構造のみを変更する場合は、既存の代表的なテストを1件実行してデグレードがないか確認する。

## 🛠️ テスト沼回避の掟 (Rules to avoid Test Swamp)

既存テストが落ちた場合、本体のコードを無闇に書き換える前に、以下の原因を順に疑うこと。

1. **環境エラー**: `QueryClientProvider` や `next/navigation` のモック設定は漏れていないか？必要なProviderがラップされているか？
2. **タイミングエラー**: 非同期処理の完了を待たずにアサーションしていないか？ `await waitFor` や `await screen.findBy...` を正しく使用しているか？
3. **仕様・型エラー**: 実装した新仕様（Propsの追加、イベント名の変更等）がテスト側の「台本」に反映されているか？
- **逃げの禁止**: `as any` や `.skip` による一時しのぎは絶対にしない。仕様が変わったのであれば、テストを新しい仕様に合わせて「書き直す」ことが正解である。