...
8. **UX & Optimistic Updates (心地よい手触りと楽観的UI)**:
   ...
   - **データフェッチ戦略: Hybrid Fetching & Background Hydration**:
      ...
      - 「大量のコンポーネントやドラッグ＆ドロップ用コンテキスト（@dnd-kit等）が一斉に切り替わるインタラクション層では、並行モードによる干渉や遅延を防ぐため、常にピュアな同期駆動とセクション別の SortableContext 分割構造を標準設計とすること。タブの切り替えアクションは 0ms で先行応答するローカルState（localScope等）を併用してボタン背景色を一瞬で切り替えること。」
