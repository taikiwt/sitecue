...
2.5. DAL (Data Access Layer) の利用徹底: DB操作を行う際は、Hooks等から直接Supabaseクライアントのメソッドを呼び出さず、必ず各アプリに配備された専用のDAL関数を経由してデータ通信を行うこと。
...
8. **UX & Optimistic Updates (心地よい手触りと楽観的UI)**:
   ...
   - **サイレント・リフェッチ (Silent Refetching) の徹底**:
    ...
    - **Hybrid Fetching Policy (Hybrid Fetching Strategy)**:
       - **初期ロード**: パフォーマンス最大化のため、初期ロード（全件取得等）では本文を除いたメタデータのみを取得（Slim Fetching）する。
       - **遅延読み込み**: コンテキスト（URL/ドメイン）が確定した後、または表示が必要になったタイミングで、その範囲の `content` を遅延フェッチ（Hydration）してメモリ上で既存データとマージする。
       - **UX**: 本文取得中もUIの操作性を損なわないよう、サイレントにマージを行うこと。
