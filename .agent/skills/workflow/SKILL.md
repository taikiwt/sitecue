...
- **Search Trigger Optimization (検索実行の意図的制御)**:
  - グローバル検索など、APIリクエストを伴う重い検索処理において、入力欄の `onChange` イベントで都度 `useSearchNotes` 等をトリガーしてはならない（意図せぬUIのチラつきやリクエスト過多を招くため）。
  - **絶対ルール**: 入力中の文字列（一時ステート）と、検索実行用のキーワード（Queryに渡すステート）は分けて管理し、必ず「Enterキーの押下」または「検索実行ボタンのクリック（`onSubmit`）」をトリガーとして明示的に検索を走らせること。
