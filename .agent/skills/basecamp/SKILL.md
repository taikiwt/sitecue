...
- **Markdown Rendering**: ノートやテキストの表示には素のテキストエリアではなく、 `MarkdownRenderer` (Tailwind Typography `prose` 適用済み) を使用し、「Studio」画面のような洗練された美しい余白とタイポグラフィを維持すること。
- **Action Buttons**: 各種アクション（完了トグル、コピー等）は楽観的UI (Optimistic UI) で実装し、アイコンやボタンは拡張機能の野暮ったいスタイルではなく、App Basecamp側の洗練されたデザインシステムに合わせること。