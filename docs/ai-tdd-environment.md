...
- **原因:** `extension/` と `apps/app/` でReactのバージョン指定がわずかに異なっていた（`^19.2.0` と `19.2.3`）ため、Bunがパッケージの巻き上げ（Hoisting）に失敗。`node_modules` と `apps/app/node_modules` の両方に別々のReactがインストールされ、Vitest実行時にメモリ上でReactが2つ衝突してパニックを起こした。
- **解決策:**
