...
- **テストの実行方法:**
    ...
    2. `cd web` に移動し、`bun run test:e2e` を実行する.
    3. （※UIモードで視覚的に確認したい場合は `bun run test:e2e --ui`）.
- **Next.jsの自動起動:** `web/playwright.config.ts` の `webServer` 設定により、テスト実行時に自動でNext.js（`bun run dev`）が立ち上がるよう構成されているため、Next.js側を事前に手動起動しておく必要はなくなりました.
